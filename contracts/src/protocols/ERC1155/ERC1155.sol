...
contract ERC1155Facet is ERC173, ERC1155 {
    ...
    function balanceOfBatch(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            balances[i] = ERC1155Lib.balanceOf(accounts[i], ids[i]);
        }
    }
    ...
    function isApprovedForAll(
        ...
        return ERC1155Lib.s().operatorApprovals[account][operator];
    }
    ...
    function safeTransferFrom(
        ...
        require(
            from == msg.sender ||
                ERC1155Lib.s().operatorApprovals[from][msg.sender],
            "ERC1155: not approved"
        );

        ERC1155Lib._transfer(from, to, id, amount);

        emit TransferSingle(msg.sender, from, to, id, amount);

        ERC1155Lib._callOnReceived(msg.sender, from, to, id, amount, data);
    }
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ERC1155Lib._transfer(from, to, ids[i], amounts[i]);
        }

        emit TransferBatch(msg.sender, from, to, ids, amounts);

        ERC1155Lib._callOnBatchReceived(
            msg.sender,
            from,
            to,
            ids,
    ...
    function mint(address to, uint256 id, uint256 amount) external onlyOwner {
        ERC1155Lib._mint(to, id, amount);
        emit TransferSingle(msg.sender, address(0), to, id, amount);
    }

    function burn(address from, uint256 id, uint256 amount) external onlyOwner {
        ERC1155Lib._burn(from, id, amount);
