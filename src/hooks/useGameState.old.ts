...
// Align with repository key for continuity
const GAME_STATE_STORAGE_KEY = 'civ_mobile_game_state';
...
export function useGameState() {
  ...
  useEffect(() => {
    ...
    const saved = loadJSON<GameState | null>(GAME_STATE_STORAGE_KEY, null);
    if (saved && saved.village && saved.playerStats && saved.techTree) {
      // Shallow-merge: prefer saved values, fall back to defaults for new fields
      const merged: GameState = {
        ...INITIAL_STATE,
        ...saved,
        village: {
          ...INITIAL_STATE.village,
          ...saved.village,
          resources: { ...INITIAL_STATE.village.resources, ...saved.village.resources },
          uncollectedResources: { ...INITIAL_STATE.village.uncollectedResources, ...saved.village.uncollectedResources },
          buildings: { ...INITIAL_STATE.village.buildings, ...saved.village.buildings },
          army: { ...INITIAL_STATE.village.army, ...saved.village.army },
          trainingQueue: saved.village.trainingQueue || [],
          lastUpdate: saved.village.lastUpdate || Date.now(),
        },
        playerStats: { ...INITIAL_STATE.playerStats, ...saved.playerStats },
        techTree: { ...INITIAL_STATE.techTree, ...saved.techTree },
        marches: saved.marches || INITIAL_STATE.marches,
        marchPresets: saved.marchPresets || INITIAL_STATE.marchPresets,
      };
      setGameState(merged);
    }
  ...
  // Persist on changes with debounce
  useEffect(() => {
    if (!hasHydratedRef.current) return; // wait until hydration to avoid overwriting saved state immediately
    if (saveTimerRef.current) {
      window.clearTimeout(saveTimerRef.current);
    }
    saveTimerRef.current = window.setTimeout(() => {
      saveJSON(GAME_STATE_STORAGE_KEY, gameState);
    }, 800);
...
// Persist to localStorage with simple debounce to reduce write frequency
export function usePersistGameState(state: GameState) {
  ...
  useEffect(() => {
    ...
    timerRef.current = window.setTimeout(() => {
      saveJSON(GAME_STATE_STORAGE_KEY, state);
    }, 800);
