...
// Helper functions
export function calculateResourceProduction(buildings: {
  ...
  Object.entries(buildings).forEach(([buildingId, building]) => {
    ...
    if (buildingDef && buildingDef.productionPerHour) {
      ...
      Object.entries(buildingProduction).forEach(([resource, amount]) => {
        production[resource as keyof Amounts] =
          (production[resource as keyof Amounts] || 0) + amount;
      });
    }
  });
...
export function calculatePopulationCapacity(buildings: {
  [key: string]: { level: number };
}): number {
