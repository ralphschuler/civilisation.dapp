...
import { Village, Building, TrainingQueue } from '@/types/game';
    ...
    (set, get) => ({
      ...
      updateVillage: async (village: Village) => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to update village' });
        }
      },
      ...
      collectResources: async (resourceType?: keyof Village['uncollectedResources']) => {
        ...
        if (resourceType) {
          ...
          (Object.keys(newUncollectedResources) as Array<keyof Village['uncollectedResources']>).forEach(key => {
