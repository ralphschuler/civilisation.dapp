...
import { TechTree, BuildingId, UnitId } from '@/types/game';
    ...
    (set, get) => ({
      ...
      updateTechTree: async (techTree: TechTree) => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to update tech tree' });
        }
      },
      ...
      unlockBuilding: async (buildingId: BuildingId) => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to unlock building' });
        }
      },
      ...
      unlockUnit: async (unitId: UnitId) => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to unlock unit' });
        }
      },

      upgradeSmithyLine: async (line: 'inf' | 'cav' | 'ranged' | 'siege', stat: 'attack' | 'defense') => {
        try {
          const repository = getRepository();
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to upgrade smithy line' });
        }
      },
      ...
      advanceEra: async (era: TechTree['era']) => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to advance era' });
        }
      },
