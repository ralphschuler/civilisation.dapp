...
export function AchievementsScreen({ onBack }: AchievementsScreenProps) {
  ...
  const formatReward = (reward: any) => {
    switch (reward.type) {
      case "gold":
        return `${reward.amount} ${t("screens.achievements.rewards.goldUnit", "Gold")}`;
      case "resource":
        return reward.value;
      case "title":
        return `${t("screens.achievements.rewards.titleLabel", "Titel")}: ${reward.value}`;
      case "bonus":
        return reward.value;
      case "unit":
        return reward.value;
      case "cosmetic":
        return reward.value;
      default:
        return t("screens.achievements.rewards.unknown", "Unbekannte Belohnung");
    }
  };
    ...
    <div className="space-y-4">
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-section font-medium">
                  {t("screens.achievements.progress", "Fortschritt")}
                </h3>
                <p className="text-caption text-muted-foreground">
                  {completedAchievements} {t("screens.achievements.of", "von")} {totalAchievements}{" "}
                  {t("screens.achievements.achievements", "Erfolgen")}
                </p>
              </div>
            </div>
      ...
      <Tabs defaultValue="achievements" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="achievements" className="text-caption">
            {t("screens.achievements.tabs.achievements", "Erfolge")}
          </TabsTrigger>
          <TabsTrigger value="quests" className="text-caption">
            {t("screens.achievements.tabs.quests", "Quests")}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="achievements" className="space-y-3">
          {achievements.map((achievement) => (
            <Card
              key={achievement.id}
              className={`relative ${achievement.locked ? "opacity-60" : ""}`}
            >
              <CardContent className="p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-body font-medium">
                        {t(`screens.achievements.items.${achievement.id}.name`, achievement.name)}
                      </h4>
                      <Badge
                        variant="outline"
                    ...
                    <p className="text-caption text-muted-foreground mb-2">
                      {achievement.locked
                        ? t(
                            `screens.achievements.items.${achievement.id}.requirement`,
                            achievement.requirement,
                          )
                        : t(
                            `screens.achievements.items.${achievement.id}.description`,
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="text-body font-medium">
                        {t(`screens.achievements.quests.${quest.id}.name`, quest.name)}
                      </h4>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      >
                        {quest.type === "daily"
                          ? t("screens.achievements.badges.daily", "Täglich")
                          : quest.type === "weekly"
                            ? t("screens.achievements.badges.weekly", "Wöchentlich")
                            : t("screens.achievements.badges.seasonal", "Saison")}
                      </Badge>
                    </div>
                  </div>
                </div>
          ...
          <Card className="bg-gradient-to-r from-accent/5 to-primary/5 border-dashed">
            <CardContent className="p-4 text-center">
              ...
              <h4 className="text-body font-medium mb-1">
                {t("screens.achievements.store.title", "Quest-Belohnungen")}
              </h4>
              <p className="text-caption text-muted-foreground mb-3">
                {t(
                  "screens.achievements.store.description",
