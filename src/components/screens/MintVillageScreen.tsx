    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="text-caption text-muted-foreground">
          {t('screens.mint.description', 'Dörfer sind NFTs. Jeder Spieler erhält beim Start ein Dorf kostenlos. Solltest du dein letztes Dorf verlieren, kannst du ein neues für 10 WORLD prägen.')}
        </CardContent>
      </Card>
        ...
        <Card className="border-success/30">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-success">
              <CheckCircle2 className="h-5 w-5" /> {t('screens.mint.success', 'Erfolgreich geprägt')}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-caption">
            {t('screens.mint.transaction', 'Transaktion')}: <a className="underline" href={`#tx:${txHash}`}>{txHash.slice(0, 10)}…</a>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <Badge variant={hasFreeMint ? 'default' : 'outline'} className="ml-2">
              {hasFreeMint ? t('screens.mint.free.available', 'Verfügbar') : t('screens.mint.free.used', 'Bereits genutzt')}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <Button className="w-full" disabled={!hasFreeMint || isMinting} onClick={onMintFree}>
            {hasFreeMint ? t('screens.mint.free.cta', 'Kostenloses Dorf prägen') : t('screens.mint.free.unavailable', 'Nicht verfügbar')}
          </Button>
          {!hasFreeMint && (
            <div className="text-xs text-muted-foreground text-center">{t('screens.mint.free.alreadyUsed', 'Dein kostenloses Mint wurde bereits verwendet.')}</div>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          {!canPay && (
            <Alert>
              ...
              <AlertDescription>
                {t('screens.mint.paid.needFunds', 'Du benötigst mindestens 10 WORLD, um ein neues Dorf zu prägen.')}
              </AlertDescription>
            </Alert>
          )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-section">{t('screens.mint.info.title', 'Was bedeutet „Dorf als NFT“?')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-caption text-muted-foreground">
          <ul className="list-disc pl-5 space-y-1">
            <li>{t('screens.mint.info.point1', 'Du besitzt dein Dorf on-chain; es ist übertragbar.')}</li>
            <li>{t('screens.mint.info.point2', 'Bei Verlust deines letzten Dorfs kannst du ein neues prägen.')}</li>
            <li>{t('screens.mint.info.point3', 'Das erste Dorf ist kostenlos, weitere kosten 10 WORLD.')}</li>
          </ul>
          {isMinting && (
