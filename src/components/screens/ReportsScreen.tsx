...
export function ReportsScreen({ reports: propReports = mockReports }: ReportsScreenProps) {
  ...
  const reports = useMemo(
    () => (storeReports.length ? storeReports : propReports),
    [storeReports, propReports],
  );
  ...
  const formatTime = (timestamp: number) => {
    ...
    if (days > 0)
      return `${t("screens.reports.time.ago", "vor")} ${days} ${t(days > 1 ? "screens.reports.time.days" : "screens.reports.time.day", days > 1 ? "Tagen" : "Tag")}`;
    if (hours > 0)
      return `${t("screens.reports.time.ago", "vor")} ${hours} ${t(hours > 1 ? "screens.reports.time.hours" : "screens.reports.time.hour", hours > 1 ? "Stunden" : "Stunde")}`;
    if (minutes > 0)
      return `${t("screens.reports.time.ago", "vor")} ${minutes} ${t(minutes > 1 ? "screens.reports.time.minutes" : "screens.reports.time.minute", minutes > 1 ? "Minuten" : "Minute")}`;
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4
                      className={`text-section leading-none ${!report.read ? "font-medium" : ""}`}
                    >
                      {report.title}
                    </h4>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate(-1)} className="min-touch">
      ...
      {unreadCount > 0 && (
        <Alert>
          ...
          <AlertDescription>
            {t("screens.reports.unreadSummary.prefix", "Du hast")} {unreadCount}{" "}
            {t("screens.reports.unreadSummary.unread", "ungelesene Berichte")}
            {importantCount > 0 && (
              <>
                ...
                <div className="space-y-3 p-4">
                  {filteredReports.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-section font-medium mb-2">
                        {t("screens.reports.empty.title", "Keine Berichte")}
                      </h3>
                      <p className="text-caption text-muted-foreground">
                        {activeTab === "unread"
                          ? t(
                              "screens.reports.empty.allRead",
                              "Alle Berichte wurden bereits gelesen.",
                            )
                          : t(
                              "screens.reports.empty.noneInCategory",
      ...
      <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
        <DialogContent className="max-w-[95vw] max-h-[90vh] overflow-y-auto">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {t("screens.reports.spy.title", "Spionage-Bericht")}:{" "}
                    {selectedReport.spyData.target}
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  {selectedReport.spyData.resources && (
                    <div>
                      <h4 className="font-medium mb-2">
                        {t("screens.reports.spy.resources", "Ressourcen")}
                      </h4>
                      <div className="grid grid-cols-2 gap-2">
                        {Object.entries(selectedReport.spyData.resources).map(
                          ([resource, amount]) => (
                            <div
                              key={resource}
                              className="flex justify-between p-2 bg-muted/30 rounded-md"
                            >
                  ...
                  {selectedReport.spyData.units && (
                    <div>
                      <h4 className="font-medium mb-2">
                        {t("screens.reports.spy.units", "Einheiten")}
                      </h4>
                      <div className="grid grid-cols-2 gap-2">
                  ...
                  {selectedReport.spyData.buildings && (
                    <div>
                      <h4 className="font-medium mb-2">
                        {t("screens.reports.spy.buildings", "Gebäude")}
                      </h4>
                      <div className="grid grid-cols-2 gap-2">
                        {Object.entries(selectedReport.spyData.buildings).map(
                          ([building, level]) => (
                            <div
                              key={building}
                              className="flex justify-between p-2 bg-muted/30 rounded-md"
                            >
                              ...
                              <span className="text-caption font-medium">
                                {t("screens.reports.spy.level", "Stufe")} {level}
                              </span>
                            </div>
                          ),
                        )}
          ...
          {!selectedReport?.battleData && !selectedReport?.spyData && selectedReport && (
            <div className="space-y-4">
              <div className="text-caption">{selectedReport.summary}</div>
              <Button onClick={() => setShowDetailDialog(false)} className="w-full">
                {t("common.close", "Schließen")}
              </Button>
            </div>
