            ...
            {Object.entries(localSettings.notifications).map(([key, value]) => (
              <div key={key} className="flex items-center justify-between">
                <Label htmlFor={`notif-${key}`} className="text-sm">
                  ...
                  {key === "buildings" &&
                    t("screens.settings.notifications.buildings", "Gebäude fertig")}
                  {key === "alliance" &&
                    t("screens.settings.notifications.alliance", "Allianz-Nachrichten")}
                  {key === "sound" &&
                    t("screens.settings.notifications.sound", "Sound-Benachrichtigungen")}
                </Label>
                <Switch
                  id={`notif-${key}`}
                  checked={value}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="de">
                    {t("screens.settings.display.languages.de", "Deutsch")}
                  </SelectItem>
                  <SelectItem value="en">
                    {t("screens.settings.display.languages.en", "English")}
                  </SelectItem>
                  <SelectItem value="fr">
                    {t("screens.settings.display.languages.fr", "Français")}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            {Object.entries(localSettings.gameplay).map(([key, value]) => (
              <div key={key} className="flex items-center justify-between">
                <Label className="text-sm">
                  {key === "autoCollectResources" &&
                    t("screens.settings.gameplay.autoCollect", "Auto-Ressourcen sammeln")}
                  {key === "confirmAttacks" &&
                    t("screens.settings.gameplay.confirmAttacks", "Angriffe bestätigen")}
                  {key === "showCoordinates" &&
                    t("screens.settings.gameplay.showCoordinates", "Koordinaten anzeigen")}
                  {key === "quickActions" &&
                    t("screens.settings.gameplay.quickActions", "Schnellaktionen")}
                </Label>
                <Switch
                  checked={value}
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex gap-3">
            ...
            <Button className="flex-1">{t("screens.settings.actions.save", "Speichern")}</Button>
          </div>
        </CardContent>
      </Card>
    </div>
